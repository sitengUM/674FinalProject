...
class Semantic3D(Dataset):
    def __init__(self, num_points, partition='train', validate=True):
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        if validate:
            self.root = os.path.join(BASE_DIR, 'data',  'semantic3D', 'full_dataset', 'train', 'clean', 'h5dir')
        else:
            self.root = os.path.join(BASE_DIR, 'data', 'semantic3D', 'full_dataset', 'test', 'clean', 'h5dir')
        self.partition = partition
        self.num_points = num_points
        ...
        for current_file in os.listdir(self.root):
            ...
            filename_to_start_index[current_path] = self.num_scene_windows

            h5f = h5py.File(current_path, 'r')
            num_windows = h5f['data'].shape[0]
            self.num_scene_windows += num_windows
            for i in range(num_windows):
                index_to_filename.append(current_path)
